...
#else
#ifdef STM32F4
            if (sdcard.useDMAForTx && DMA_GetFlagStatus(sdcard.dma->ref, dmaFlag_IT_TCIF(sdcard.dma->ref)) == SET) {
                DMA_ClearFlag(sdcard.dma->ref, dmaFlag_IT_TCIF(sdcard.dma->ref));
#else
            if (sdcard.useDMAForTx && DMA_GetFlagStatus(dmaFlag_IT_TCIF(sdcard.dma->ref)) == SET) {
                DMA_ClearFlag(dmaFlag_IT_TCIF(sdcard.dma->ref));
#endif
